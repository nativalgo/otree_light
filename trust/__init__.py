...
def calculate_game_payoff(player: Player):
    ...
    participant.payoff_trust = sum([p.payoff for p in player.in_all_rounds()])
...
class SendBack(Page):
    ...
    def vars_for_template(player: Player):
        group = player.group
        tripled_amount = group.sent_back_amount_max(group.sent_amount)
...
class Task2Conclusion(Page):
    form_model = 'player'
page_sequence = [Introduction, Decisions, AssignmentA, AssignmentB, Send, SendWaitPage, SendBack, SendBackWaitPage, ResultsWaitPage, Results, Task2Conclusion]